...
export default function GastosPage() {
  ...
  const handleAdd = (e: React.FormEvent) => {
    ...
    gastosService.add({ concepto: concepto.trim(), monto: m, fecha });
    ...
    <div>
      <div className="flex items-center justify-between mb-6 flex-wrap gap-2">
        <div>
          ...
          <p className="text-gray-500 text-sm">Registro de todos tus gastos</p>
        </div>
        <button id="btnNuevoGasto" onClick={() => setModalOpen(true)}
          className="flex items-center gap-2 bg-[#e74a3b] hover:bg-[#c0392b] text-white font-semibold px-4 py-2.5 rounded-xl shadow-md shadow-red-500/20 transition-all duration-200">
          <i className="fas fa-plus"></i> Nuevo Gasto
        </button>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        <StatCard label="Total Gastos" value={fmt(total)} icon="fas fa-receipt"
          accentColor="border-l-[#e74a3b]" iconColor="text-[#e74a3b]" />
      </div>

      <div className="bg-white rounded-2xl shadow-[0_0.15rem_1.75rem_rgba(58,59,69,0.08)] overflow-hidden">
        <div className="px-5 py-4 border-b border-gray-100 flex items-center gap-2">
          <i className="fas fa-list text-[#e74a3b]"></i>
          <h6 className="font-bold text-gray-700">Historial de Gastos</h6>
        </div>

        {gastos.length === 0 ? (
          <div className="text-center py-12 text-gray-400">
            <i className="fas fa-inbox text-4xl mb-3 opacity-25"></i>
            <p className="mb-3">No hay gastos registrados aún.</p>
            <button onClick={() => setModalOpen(true)}
              className="inline-flex items-center gap-2 text-sm bg-[#e74a3b] text-white px-4 py-2 rounded-lg hover:bg-[#c0392b] transition">
              ...
              <tbody className="divide-y divide-gray-100">
                {gastos.map((item) => (
                  <tr key={item.id} className="hover:bg-gray-50 transition">
                    ...
                    <td className="px-5 py-3.5 font-bold text-[#e74a3b]">{fmt(item.monto)}</td>
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md">
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
              <h5 className="font-bold text-gray-800 flex items-center gap-2">
                <i className="fas fa-plus-circle text-[#e74a3b]"></i> Nuevo Gasto
              </h5>
            ...
            <form onSubmit={handleAdd} className="px-6 py-5 space-y-4">
              <div>
                ...
                <input id="inputConcepto" type="text" required value={concepto} onChange={e => setConcepto(e.target.value)}
                  ...
                  className="w-full px-4 py-2.5 rounded-xl border border-gray-200 focus:outline-none focus:ring-2 focus:ring-[#e74a3b] transition text-sm" />
              </div>
              <div>
              ...
              <button type="submit"
                className="w-full py-3 rounded-xl bg-[#e74a3b] hover:bg-[#c0392b] text-white font-bold transition flex items-center justify-center gap-2">
                <i className="fas fa-check"></i> Guardar Gasto
              </button>
            </form>
          </div>
        </div>
